...
//Guest notes
router.post('/guests/notes', async (req, res) => {
    try {
        const { title, content, ...rest } = req.body;
        const savedNote = await Note.create({ title, content, ...rest, isShared: false });
        res.json(savedNote);
    } catch (err) {
        res.status(500).json({ error: "Failed to store guest note" });
    }
});
